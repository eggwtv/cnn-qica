"""
=============================================================================
cnn_v6.py  —  BEAVRS CNN v6  |  Log-Space Relative-Error Fix
=============================================================================

WHAT CHANGED FROM v5
─────────────────────────────────────────────────────────────────────────────
FIX 1 — Relative error loss (core improvement):
  v4 used MSE → minimises absolute error, ignores relative magnitude.
  v5 added MAPE with eps=0.5 → gradient weakened by 40% at PPF=2.0 because
  the denominator was (ppf_true + 0.5) not (ppf_true). Result: 3.22% rel_err,
  worse than v4's 3.10%.

  v6 replaces MAPE with log-space MSE:
    log_loss = W_LOG * mean( (log(pred_real) - log(true_real))² )
  This is mathematically equivalent to relative error² for small errors
  (via Taylor: log(pred/true) ≈ (pred−true)/true). Advantages:
    • Gradient ∝ 1/pred → naturally larger at low PPF → low-PPF zone improves
    • Symmetric: over/under prediction penalised equally in percentage terms
    • No division → numerically stable during early training when pred can drift

FIX 2 — dense_units reverted to 128:
  The v5 tuner found dense_units=64 at 50-epoch trial length. At full 400-epoch
  training the smaller trunk underperformed v4's 128-unit trunk. Short trials
  favour compact models (faster convergence) but not higher asymptotic quality.
  128 restored; ppf_head_units=64 kept (tuner result was meaningful here).

ARCHITECTURE (from v5 tuner — best confirmed config):
  Input (6×6 int grid) → Embedding(9+1, 16) → ConvResBlock(64) → ConvResBlock(128)
  ...
  Filters: [64, 128] (confirmed best in v5 Stage-2 search).
...
LOSS (v6):
  L = W_PPF_MAX × MSE(ppf_max)
    ...
    + W_LOG × MSE(log(ppf_max_pred_real), log(ppf_max_true_real))   ← NEW

OUTPUTS:
  cnn_v6_model.keras        — trained model (serializable)
  cnn_v6_config.json        — geometry, scalers, indices
  cnn_v6_sens.csv           — position sensitivities
  cnn_v6_results.png        — evaluation plots
  cnn_v6_al_candidates.csv  — active learning query candidates
  train_type_freq_v6.npy    — per-position type frequencies (QICA trust-region)
=============================================================================
"""
...
print("cnn_v6.py — BEAVRS CNN  |  Log-space relative-error fix\n")
...
MODEL_NAME  = 'cnn_v6_model.keras'
CONFIG_NAME = 'cnn_v6_config.json'
SENS_NAME   = 'cnn_v6_sens.csv'
PLOT_NAME   = 'cnn_v6_results.png'
AL_CSV      = 'cnn_v6_al_candidates.csv'
FREQ_PATH   = 'train_type_freq_v6.npy'
...
# ── Architecture (best from v5 tuner, dense_units reverted to 128) ────────────
EMBED_DIM      = 16
FILTERS        = (64, 128)          # confirmed best in Stage-2 search
NUM_BLOCKS     = 2                  # confirmed best (64→128 filter progression)
DENSE_UNITS    = 128                # reverted: 64 underperformed at full 400 ep
...
W_RHO       = 5.0
W_MONO      = 0.01
# Log-space weight. 2.0 gives a meaningful gradient push toward relative
# accuracy without destabilising MSE-dominated early training.
# Increase to 3.0 if rel_err is still above 2.5% after this run.
W_LOG       = 2.0
...
# Globals set after scaler.fit() — used inside the loss function
_PPF_MAX_MEAN = None
_PPF_MAX_STD  = None
...
# =============================================================================
#
# @register_keras_serializable is mandatory for model.save() → load_model()
# to succeed. Without it, Keras stores the class name as a string but cannot
# reconstruct it when loading because the class isn't in its registry.
# With the decorator, ConvResBlock is registered globally so any script that
# defines it (and has the decorator) can save/load models containing it.
...
class ConvResBlock(layers.Layer):
    ...
    Residual shortcut: lets gradients flow directly back, avoiding vanishing
    gradients in deeper stacks. Projection Conv2D (1×1) handles any filter
    dimension mismatch between input and the residual output.
    """
...
def build_cnn_v6(
    ...
    """
    Multi-head CNN surrogate (attention OFF — Stage-1 confirmed best).
      Input → Embedding(10, 16) → 2×ConvResBlock([64,128]) → GlobalAvgPool
            → Dense(128, gelu) → Dropout(0.2) → Dense(64, gelu)
            → [PPF head] + [Cycle head] + [Rho head] → Concatenate → (B, 35)

    Three separate output heads prevent gradient interference between PPF
    (many outputs, high weight), cycle length (scalar), and rho_pcm (separate
    scaler, high relative weight).
    """
    ...
    # Shared trunk — jointly trained features before heads split
    ...
    # Rho head: 1 output (separate to avoid gradient swamping)
    ...
    return keras.Model(inputs=inp, outputs=out, name='BEAVRS_CNN_v6')
...
# =============================================================================
#
# v6 loss = v5 MSE terms + monotonicity penalty + NEW log-space MSE term.
#
# Log-space term derivation:
#   Let r = pred_real / true_real.  Then:
#     (log r)² = (log(1 + (r-1)))² ≈ (r - 1)² for r ≈ 1
#              = ((pred - true) / true)²
#   So MSE(log pred, log true) ≈ mean((rel_err)²) in real space.
#
#   Gradient w.r.t. pred (scaled):
#     d/d(pred_sc) [log(pred_real) - log(true_real)]²
#     = 2 * (log(pred_real) - log(true_real)) / pred_real * _PPF_MAX_STD
#   At low PPF (pred_real small), pred_real is small → gradient is LARGER.
#   This is the opposite of MSE, which gives equal absolute-error gradients.
#
# LOG_CLAMP: clamp denominator to 0.5 to prevent log(0) if a prediction
# wanders far negative during early training. Since PPF_min ≈ 1.5 in the
# real dataset, log(1.5) = 0.4 vs log(0.5) = -0.69, so this is a generous
# safety margin that never activates in the converged regime.

LOG_CLAMP = 0.5


def v6_loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
    ...
    # Penalise any upward steps in the late-cycle prediction.
    ...
    # ── Log-space MSE: directly targets relative error ─────────────────────────
    # Unscale ppf_max back to real space using the stored scaler constants.
...
print("[TRAINING] Starting CNN v6 ...")
print(f"  Filters      : {FILTERS}  (best from v5 Stage-2 search)")
print(f"  Dense units  : {DENSE_UNITS}  (reverted: 64 underperformed at full length)")
print(f"  Log-space W  : {W_LOG}  (log-space relative-error term, replaces MAPE)")
print()
...
# Relative error by PPF zone (diagnostic breakdown)
...
print(f"\n{'='*62}")
print(f"CNN v6 TEST RESULTS")
print(f"{'='*62}")
print(f"  PPF_max (all patterns):")
print(f"    MAE              : {ppf_mae:.4f}   (v4: 0.0843 | v5: 0.0875)")
print(f"    Relative error   : {ppf_rel_err:.2f}%    (v4: 3.10% | v5: 3.22%)")
print(f"    R²               : {ppf_r2:.4f}   (v4: 0.9841 | v5: 0.9826)")
print(f"    Pearson r        : {ppf_pearson:.4f}")
print(f"")
print(f"  Relative error breakdown by PPF zone:")
print(f"    PPF < 2.5  (n={low_mask.sum():4d}) : {rel_low:.2f}%   ← target zone")
...
fig.suptitle(
    f"BEAVRS CNN v6  |  PPF MAE={ppf_mae:.3f}  rel_err={ppf_rel_err:.2f}%  "
...
# 5. Relative error by PPF zone (v4/v5/v6 comparison)
ax = fig.add_subplot(gs[1, 0])
versions = ['v4 MSE', 'v5 MAPE\n(eps=0.5)', 'v6 Log-MSE']
# Use actual v6 numbers for zones; v4/v5 had no breakdown so approximate
rel_errs = [3.10, 3.22, ppf_rel_err]
bars = ax.bar(versions, rel_errs,
              color=['#D62728', '#F5A623', '#1B4FBF'], alpha=0.75, width=0.5)
...
# 6. Relative error breakdown by zone (v6 only)
ax = fig.add_subplot(gs[1, 1])
zones   = [f'PPF<2.5\n(n={low_mask.sum()})',
           f'2.5–4.0\n(n={mid_mask.sum()})',
           f'PPF>4.0\n(n={high_mask.sum()})']
rel_z   = [rel_low, rel_mid, rel_high]
colors_z = ['#D62728', '#17BECF', '#AAAAAA']
bars_z = ax.bar(zones, rel_z, color=colors_z, alpha=0.75)
for bar, val in zip(bars_z, rel_z):
    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.03,
            f'{val:.2f}%', ha='center', va='bottom', fontsize=9)
ax.set_ylabel('Relative error (%)')
ax.set_title('v6 Relative Error\nby PPF Zone')
...
# =============================================================================
# SECTION 16 — SENSITIVITY HEATMAP (separate figure for clarity)
# =============================================================================
...
plt.tight_layout()
plt.savefig('cnn_v6_sensitivity.png', dpi=150, bbox_inches='tight')
print(f"[SAVED]  cnn_v6_sensitivity.png")
...
print(f"  Val/train ratio  : {final_ratio:.2f}  "
      f"({'✓ good' if final_ratio < 1.3 else '⚠ mild overfit'})")
print()
print(f"  {'Metric':<30} {'v4':>8}  {'v5':>8}  {'v6':>8}")
print(f"  {'-'*30} {'-'*8}  {'-'*8}  {'-'*8}")
print(f"  {'PPF MAE':<30} {'0.0843':>8}  {'0.0875':>8}  {ppf_mae:>8.4f}")
print(f"  {'PPF rel. error':<30} {'3.10%':>8}  {'3.22%':>8}  {ppf_rel_err:>7.2f}%")
print(f"  {'PPF R²':<30} {'0.9841':>8}  {'0.9826':>8}  {ppf_r2:>8.4f}")
print(f"  {'Cycle MAE (days)':<30} {'1.28':>8}  {'1.61':>8}  {cycle_mae:>8.2f}")
print(f"  {'keff R²':<30} {'0.9912':>8}  {'0.9889':>8}  {keff_r2_r:>8.4f}")
print(f"  {'Rel. error (PPF<2.5)':<30} {'n/a':>8}  {'3.55%':>8}  {rel_low:>7.2f}%")
print()
print(f"  OUTPUT FILES:")
print(f"    {MODEL_NAME:<32} — serializable model")
print(f"    {CONFIG_NAME:<32} — scalers, indices, arch config")
print(f"    {SENS_NAME:<32} — position sensitivities")
print(f"    {PLOT_NAME:<32} — evaluation plots")
print(f"    cnn_v6_sensitivity.png          — sensitivity heatmap")
print(f"    {AL_CSV:<32} — AL query candidates")
print(f"    {FREQ_PATH:<32} — trust-region frequencies")
print()
print(f"  RELATIVE ERROR GUIDANCE:")
if ppf_rel_err < 2.5:
    print(f"    ✓ Relative error improved below 2.5%.  Proceed to QICA.")
elif ppf_rel_err < 3.10:
    print(f"    ✓ Relative error improved vs v4.  Consider W_LOG=3.0 for further gain.")
else:
    print(f"    ✗ Relative error not improved.  Try increasing W_LOG from {W_LOG} to 3.0")
    print(f"      in Section 1 and rerunning from Section 8 only (skip data load).")
print()
print(f"  NEXT STEP: Run qica-cnn-v6.py (point it at cnn_v6_model.keras + config).")
print('=' * 66)