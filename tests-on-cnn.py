"""
=============================================================================
tests-on-cnn.py  —  BEAVRS CNN v4  |  Fixed keff  |  Min-PPF Surrogate
=============================================================================
...
print("cnn_v4.py — BEAVRS CNN  |  Fixed keff  |  Min-PPF Surrogate\n")
...
MODEL_NAME  = 'cnn_v4_model.keras'
CONFIG_NAME = 'cnn_v4_config.json'
SENS_NAME   = 'cnn_v4_sens.csv'
PLOT_NAME   = 'cnn_v4_results.png'
AL_CSV      = 'cnn_v4_al_candidates.csv'
FREQ_PATH   = 'train_type_freq.npy'
...
def build_cnn_v4(
    ...
    return keras.Model(inputs=inp, outputs=out, name='BEAVRS_CNN_v4')
...
# =============================================================================
# BUILD MODEL (CHANGED → SINGLE OUTPUT HEAD)
# =============================================================================

def build_cnn_v4():
    inp = keras.Input(shape=(GRID_ROWS, GRID_COLS), dtype='int32')

    x = layers.Embedding(N_TYPES + 1, 16)(inp)

    x = ConvResBlock(32, dropout=CONV_DROP)(x)
    x = ConvResBlock(64, dropout=CONV_DROP)(x)

    # ❌ REMOVED SPATIAL ATTENTION (commented)
    ...
    # ✅ SINGLE SHARED OUTPUT HEAD (REPLACES MULTI-HEAD)
    outputs = layers.Dense(N_STEPS + 3, activation='linear', name='predictions')(x)
...
def v4_loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
...
def build_cnn_tunable(hp):
    ...
    conv_dropout = hp.Choice(
        "conv_dropout",
        [0.0, 0.05, 0.10, 0.15]
    )

    num_blocks = hp.Choice(
        "num_blocks",
        [1, 2, 3]
    )
    ...
    multi_head = hp.Boolean(
        "multi_head"
    )

    use_attention = hp.Boolean(
        "use_attention"
    )
    ...
    # --------------------------------------------------
    # MULTI-HEAD OPTION filter_sizes
    # --------------------------------------------------
    ...
    if multi_head:
        ...
        h_ppf = layers.Dropout(
            dropout * 0.5
        )(h_ppf)

        out_ppf = layers.Dense(
            33,
            activation="linear"
        )(h_ppf)

        h_cycle = layers.Dense(
            32,
            activation="gelu"
        )(shared)

        out_cycle = layers.Dense(
            1,
            activation="linear"
        )(h_cycle)
...
'''
def build_stage2_tuner(hp):

    # FIXED from your best run
    embed_dim = 16
    dense_units = 128
    dropout = 0.2
    conv_dropout = 0.1
    ...
    inp = keras.Input(shape=(GRID_ROWS, GRID_COLS), dtype="int32")
    x = layers.Embedding(N_TYPES + 1, embed_dim)(inp)

    # build conv blocks
    for i in range(num_blocks):
        f = filters[i] if i < len(filters) else filters[-1]
        x = ConvResBlock(f, dropout=conv_dropout)(x)

    x = layers.GlobalAveragePooling2D()(x)

    shared = layers.Dense(dense_units, activation="gelu")(x)
    shared = layers.Dropout(dropout)(shared)

    # multi-head FIXED (always on now)
    h_ppf = layers.Dense(64, activation="gelu")(shared)
    out_ppf = layers.Dense(33)(h_ppf)

    h_cycle = layers.Dense(32, activation="gelu")(shared)
    out_cycle = layers.Dense(1)(h_cycle)

    h_rho = layers.Dense(32, activation="gelu")(shared)
    out_rho = layers.Dense(1)(h_rho)

    out = layers.Concatenate()([out_ppf, out_cycle, out_rho])
...
tuner2 = kt.RandomSearch(
    build_stage2_tuner,
    objective="val_loss",
    max_trials=6,   # VERY IMPORTANT (keeps runtime low)
    directory="cnn_stage2",
    project_name="conv_structure_search"
)
...
tuner2.search(
    X_tr, Y_tr_sc,
    validation_data=(X_val, Y_val_sc),
    epochs=40,          # MUCH smaller than 80–200
    batch_size=128,
    callbacks=[stop],
    verbose=1
)
...
model = tuner2.hypermodel.build(best_hp2)
# =============================================================================
# SECTION 8 — TRAIN
# =============================================================================
#
...
callbacks = [
    ...
    keras.callbacks.ReduceLROnPlateau(patience=15)
]

print("[TRAINING] Starting ...")
print("[TRAINING] Starting CNN v4 ...")
...
print("=" * 62)
print(f"  Architecture     : BEAVRS CNN v4 (multi-head, fixed keff)")
