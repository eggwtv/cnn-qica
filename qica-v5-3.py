"""
problem is that your current compute_mc_entropy() is actually just a log-transformed σ
So it contains exactly the same information as σ.
...
def compute_histogram_entropy(mc_preds: np.ndarray,
                              bins: int = 15) -> np.ndarray:
    """
    True Shannon entropy of MC prediction distribution.

    Args:
        mc_preds : (MC_SAMPLES, N)

    Returns:
        entropy : (N,)
    """

    n_patterns = mc_preds.shape[1]
    entropy = np.zeros(n_patterns, dtype=np.float32)
...
def analyze_trust_region(freq: np.ndarray, free_frac: float = ENTROPY_FREE_FRAC):
    ...
    """
    h_pos      = compute_histogram_entropy(freq)
...
def evaluate_batch(patterns_int: np.ndarray) -> dict:
    ...
    if B == 1:
        ...
        plt.hist(mc_ppf_phys[:,0], bins=15)
        plt.xlabel("PPF prediction")
        plt.ylabel("Count")
        plt.title("MC Dropout Predictive Distribution")
        plt.tight_layout()
        plt.savefig("mc_entropy_hist.png")
        plt.close()
    ppf_entropy = compute_histogram_entropy(
        mc_ppf_phys,
        bins=15
    )
