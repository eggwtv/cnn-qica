"""
=============================================================================
qica_v8.py  —  Multi-Entropy QICA  |  Entropy-Method Showdown  [FIXED]
=============================================================================
PURPOSE OF THIS VERSION: answer one question with real data, in ONE run —
"which Shannon entropy formulation (if any) is actually worth keeping over
plain QICA (lowest-PPF-only candidate selection, no entropy at all)?"

KEY INSIGHT THAT MAKES THIS CHEAP: in this codebase entropy never touches
the search itself (fitness = cycle/PPF/σ/trust/monotonicity only — H_gauss,
H_hist, H_traj are computed for every evaluated country but only used to
flag Active-Learning candidates). That means the optimizer's trajectory is
IDENTICAL regardless of which entropy metric you'd use for AL. So instead
of running the full optimizer 5 times (5x runtime, plus seed-luck
confounding which method "wins"), v8 runs the optimizer ONCE, logs every
distinct pattern it ever evaluated into a shared "universe" pool, then
POST-HOC compares 5 candidate-selection rules on that exact same pool:
  1. baseline   — lowest PPF only (no entropy at all = "plain QICA")
  2. sigma-only — highest σ (MC Dropout std) among low-PPF patterns
  3. H_gauss    — highest Gaussian entropy (expect ≈ same picks as sigma-only,
                  since H_gauss is a deterministic monotonic transform of σ)
  4. H_hist     — highest calibrated histogram entropy
  5. H_traj     — highest trajectory (burnup-curve) entropy
  6. combined   — H_hist + H_traj z-score combination
This produces a DATA-DRIVEN verdict (not a hand-wave) on which methods earn
their keep, printed at the end as "KEEP" / "DROP".

This also fixes two bugs surfaced in the v7 run:
  FIX 9  — pattern_diversity ("fraction unique") is a useless metric at this
           state-space size (9^20): it reads ≈1.0 whether or not the search
           is actually exploring, which is why adaptive revolution never
           fired. Replaced with mean pairwise Hamming distance over the
           free positions — a real measure of genotype spread.
  FIX 10 — stagnation injection maxed its escalation cap and never escaped
           (v7 run: stuck at PPF=1.920 from gen 100 onward, round_idx
           reaching 10 with escalation pinned at 4). Added a hard-reset
           escape valve: after HARD_RESET_AT_CAP_COUNT consecutive
           cap-level injections with no improvement, the worst-performing
           empire is fully reinitialized (not just mutated) to actually
           leave the basin instead of being repeatedly nudged around it.

AL candidate output is capped at AL_TOP_K (default 60, per request) in the
final saved CSV — the "AL=NNN" counter printed during the run is just the
running size of the raw candidate pool before that final cap is applied.

Carries forward all v6/v7 fixes (calibrated histogram bins, percentile AL
threshold, escalating multi-elite injection, trust-region trajectory
entropy).

INPUTS:   cnn_v9_model.keras, cnn_v9_config.json, train_type_freq_v9.npy
OUTPUTS:  qica_v8_best_patterns.csv, qica_v8_al_candidates.csv,
          qica_v8_convergence.png, qica_v8_entropy_comparison.png,
          qica_v8_entropy_showdown.csv, qica_v8_entropy_showdown.png
=============================================================================
"""
...
print("qica_v8.py  —  Multi-Entropy QICA  |  Calibrated Shannon Entropy  [FIXED]\n")
...
# ─── AL thresholds ────────────────────────────────────────────────────────────
# FIX 7: AL flagging is now percentile-based on the CURRENT batch's H_hist
# distribution (self-calibrating), not a hand-tuned absolute number.
...
def is_al_candidate(h_hist_val: float, al_hist_thr: float) -> bool:
    """FIX 7: purely adaptive — caller supplies a percentile-derived threshold."""
    return float(h_hist_val) >= al_hist_thr
...
class QICAv8:
    ...
    def _evaluate_all(self, countries: list, temperature: float,
        ...
        # FIX 7: AL threshold purely adaptive — percentile of THIS batch's
        # H_hist (self-calibrating to whatever the entropy scale actually is,
        # instead of a stale absolute number tuned for a different scale).
        al_hist_thr = max(AL_HIST_MIN_FLOOR, float(np.percentile(h_hist_arr, AL_HIST_PERCENTILE)))
        self.last_al_hist_thr = al_hist_thr
        ...
        for i, c in enumerate(countries):
            ...
            # FIX 11: log EVERY evaluated country into the universe pool —
            # this is what enables the post-hoc entropy-method showdown
            # without re-running the optimizer once per entropy variant.
            ...
            # AL flagging: uncertain AND promising (live collection, capped)
            is_uncertain = is_al_candidate(c.h_hist, al_hist_thr)
    ...
    def run(self) -> dict:
        print("=" * 74)
        print(f"QICA-v8  [FIXED]  ENTROPY_MODE='{self.entropy_mode}'  "
...
def print_benchmark_comparison(best_ppf: float, best_cycle: float):
    ...
    print(f"  {'─'*27} {'─'*5} {'─'*8} {'─'*8}")
    print(f"  ► CNN + QICA-v8 (this work)   —    {best_ppf:>8.4f} {best_cycle:>8.1f} {'193':>7} 4-loop BEAVRS")
...
if __name__ == '__main__':
    ...
    if showdown:
        ...
        showdown_df.to_csv('qica_v8_entropy_showdown.csv', index=False)
        print(f"\n[SAVED]  qica_v8_entropy_showdown.csv  ({len(showdown_df)} rules compared)")
    ...
    for rank, (fit, pat, ppf, cyc, sig, hg, hh, ht) in enumerate(elite[:5], 1):
        ...
        al_h = "H⚠" if hh >= al_hist_thr_final else "H✓"
    ...
    best_df = pd.DataFrame([
        {'rank': i+1, 'ppf_max': ppf, 'sigma_ppf': sig,
         ...
         'al_hist_flag': hh >= al_hist_thr_final,
    ...
    best_df.to_csv('qica_v8_best_patterns.csv', index=False)
    print(f"\n[SAVED]  qica_v8_best_patterns.csv  ({len(best_df)} patterns)")

    if al_cands:
        al_df = (pd.DataFrame(al_cands)
                 .sort_values('priority', ascending=False)
                 .head(AL_TOP_K))
        al_df.to_csv('qica_v8_al_candidates.csv', index=False)
        top = al_df.iloc[0]
        print(f"[SAVED]  qica_v8_al_candidates.csv  ({len(al_df)} candidates)")
    ...
    fig.suptitle(
        f"QICA-v8 [FIXED]  ENTROPY_MODE='{ENTROPY_MODE}'  COMPUTE_TRAJECTORY={COMPUTE_TRAJECTORY}\n"
    ...
    plt.savefig('qica_v8_convergence.png', dpi=150, bbox_inches='tight')
    print("\n[SAVED]  qica_v8_convergence.png")
    ...
    if al_cands:
        ...
        plt.tight_layout()
        plt.savefig('qica_v8_entropy_comparison.png', dpi=150, bbox_inches='tight')
        print("[SAVED]  qica_v8_entropy_comparison.png")
    ...
    if showdown:
        ...
        plt.tight_layout()
        plt.savefig('qica_v8_entropy_showdown.png', dpi=150, bbox_inches='tight')
        print("[SAVED]  qica_v8_entropy_showdown.png")

    # ── Final summary ─────────────────────────────────────────────────────────
    print("\n" + "=" * 74)
    print("QICA-v8  FINAL SUMMARY  [ALL FIXES APPLIED]")
    print("=" * 74)
    print(f"  FIXES (v7 → v8):")
    print(f"    FIX 9  Diversity metric    : Hamming distance (free positions), replacing")
    print(f"                                 the saturated 'fraction unique' (was stuck at 1.000)")
    print(f"    FIX 10 Hard-reset escape   : full empire reinit after "
          f"{HARD_RESET_AT_CAP_COUNT} consecutive cap-level")
    print(f"                                 stagnation injections with no improvement")
    print(f"    FIX 11 Entropy showdown    : single-run, post-hoc comparison of baseline / "
          f"σ-only /")
    print(f"                                 H_gauss / H_hist / H_traj / combined AL selection rules")
    print()
    if showdown and showdown.get('verdicts'):
        print(f"  DATA-DRIVEN VERDICTS (this run):")
        for label, v in showdown['verdicts'].items():
            print(f"    {label:<42} → {v}")
        print(f"\n  → Use the qica_v8_entropy_showdown.csv / .png to decide what ships in the")
        print(f"    next version. Anything marked DROP above is not earning its keep over")
        print(f"    plain lowest-PPF selection on this run's evidence.")
    print()
    print(f"  RESULTS:")
    ...
    print(f"    AL candidates  : {len(al_cands)} unique  (threshold used: {al_hist_thr_final:.4f})")
    print(f"    Max stagnation escalation round reached: {max(hist['stagnation_round']) if hist['stagnation_round'] else 0}")
    print()
    print(f"  PAPER CONTRIBUTIONS (v7):")
    ...
    print(f"    3. Escalating multi-elite stagnation escape: novel for nuclear fuel opt.")
    print("=" * 74)