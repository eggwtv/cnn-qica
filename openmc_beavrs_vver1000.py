...
def build_materials_with_boron(type_id, boron_ppm):
    ...
    for the critical search below. Calls the ORIGINAL build_materials via
    _ORIGINAL_build_materials (captured once at import time) rather than
    the module-level `build_materials` name, which gets temporarily
    monkeypatched during boron search / boron-fixed runs — calling the
    live name here would recurse into itself infinitely."""
...
def _run_static_keff(pattern_flat, boron_ppm, particles, batches, inactive, work_dir):
    ...
    os.chdir(work_dir)
    try:
        # Monkey-patch build_materials for this call only, so every assembly
        # type in the core picks up the trial boron concentration.
...
def find_critical_boron(pattern_flat, particles=1000, batches=25, inactive=10,
                         ...
                         work_dir='boron_search', boron_lo=200.0, boron_hi=2500.0):
    """
    Secant-method search for the soluble boron ppm that brings this loading
    pattern's BOC k-eff to target_keff (default 1.000, i.e. exactly critical
    — the standard PWR/VVER convention for reporting PPF at BOC).

    Each iteration is a SHORT static transport solve (particles/batches much
    lower than your PPF-quality quick_check) — this only needs to locate
    k-eff to within ~tol_pcm, not to full statistical precision, since it's
    just tuning an input, not the final answer.

    Returns (boron_ppm_critical, keff_at_that_ppm).
    """
    ...
    if not (k_lo >= target_keff >= k_hi):
        print(f'  [WARN] target k-eff not bracketed by [{b_lo:.0f}, {b_hi:.0f}] ppm '
              f'(k_lo={k_lo:.5f}, k_hi={k_hi:.5f}). Widen boron_lo/boron_hi and retry. '
              f'Returning nearest endpoint as a rough estimate.')
        return (b_lo, k_lo) if abs(k_lo - target_keff) < abs(k_hi - target_keff) else (b_hi, k_hi)

    b_a, k_a, b_b, k_b = b_lo, k_lo, b_hi, k_hi
    for it in range(3, max_iter + 1):
        # Linear interpolation in (boron, keff) space
...
def compute_ppf_per_assembly(pin_power_mesh_values, n_pins_x, n_pins_y):
    ...
    from the mesh's lower_left — i.e. row 0 of a naive reshape = MINIMUM y).

    CRITICAL FIX: openmc.RectLattice indexes its `universes` array with
    row 0 = MAXIMUM y (image/matrix convention), which is what
    build_core_geometry() uses to place assembly types by GRID_LAYOUT row.
    openmc.RegularMesh does NOT use that convention — its natural reshape
    has row 0 = MINIMUM y. Reading the pin-power mesh back with a naive
    reshape (no flip) therefore attributes each assembly-sized block to
    the WRONG GRID_LAYOUT row (mirrored top<->bottom), which is why you
    saw exact 0.000 PPF at some real fuel positions (misaligned onto the
    reflector/inactive rows) and an inflated 6.3 max (misaligned onto a
    different assembly's true hot spot). Flipping the mesh's row axis
    before slicing puts it back in the same row-0-is-max-y convention
    that GRID_LAYOUT/build_core_geometry uses.
    ...
    grid2d = grid2d[::-1, :]   # flip: row 0 -> max y, matching RectLattice convention
    ...
    if zero_block_positions:
        ...
        print('         This means the pin-power mesh block landed entirely on non-fuel '
              '(guide tubes only / reflector) — if this recurs after the row-flip fix, '
              'check ASSEMBLY_PITCH/PIN_PITCH*PINS_PER_SIDE alignment against pin_mesh bounds.')
    return ppf_flat
...
def run_quick_check(pattern_flat, particles=2000, batches=40, inactive=15,
                     work_dir='openmc_quickcheck', boron_search=False, boron_ppm=800.0):
    """
    ...
    boron_search=True runs a short critical-boron search FIRST (a few
    cheap trial solves) and uses that boron ppm for the reported PPF,
    instead of the fixed default (800 ppm), which is the right thing to
    do if you care about PPF at a realistic (critical) operating point
    rather than whatever k-eff the fixed boron guess happens to produce.
    """
    if boron_search:
        boron_ppm, keff_est = find_critical_boron(
        ...
        print(f'[BORON SEARCH] using boron={boron_ppm:.0f} ppm '
              f'(est. k-eff={keff_est:.5f}) for the PPF report below')
    ...
    try:
        global build_materials
        orig_build_materials = build_materials
        if boron_search or boron_ppm != 800.0:
            build_materials = lambda t: build_materials_with_boron(t, boron_ppm)
        try:
            assembly_universes = {t: build_assembly_universe(t) for t in range(1, N_TYPES + 1)}
            reflector_u = build_reflector_universe()
...
def main():
    ...
    ap.add_argument('--boron_search', action='store_true',
                     help='Before reporting PPF, run a short secant search for the '
                          'soluble boron ppm that makes this pattern exactly critical '
                          '(k-eff~=1.000), matching how real PWR/VVER PPF is normalized. '
                          'Only applies with --quick_check currently.')
    ap.add_argument('--boron_ppm', type=float, default=800.0,
