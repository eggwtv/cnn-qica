...
def evaluate_batch(patterns: np.ndarray, mc_n: int = TEST_MC) -> dict:
    """Always computes H_hist and H_traj; caller decides which to use for AL."""
    if patterns.ndim == 1:
...
class AblationQICA:
    ...
    def _hard_reset(self, empires, temperature) -> bool:
        """
        FIXED VERSION vs v9:
          Original: silently returns if len(empires) < 2 — so once the search
          collapses to a single empire (gen ~50-80 in short runs), the hard
          reset NEVER fires despite stagnation_rounds repeatedly hitting the cap.
          Fix: when single empire, reinit the worst N//2 colonies instead.
        """
        ...
        if len(empires) == 1:
            ...
            # Sort ascending (worst first), reinit worst half
            emp.colonies.sort(key=lambda c: c.fitness)
            n_reinit = max(2, len(emp.colonies) // 2)
            new_c    = []
            ...
            if self.elite_archive:
                ...
                if self.flags['use_injection'] and self.stagnation_count >= STAGNATION_PATIENCE:
                    ...
                    if self.rounds_at_cap >= HARD_RESET_AT_CAP:
                        fired = self._hard_reset(empires, temp)
                        if fired:
                            reset_fired = True
                            self.rounds_at_cap = 0
...
summary_df.to_csv('test_qica_summary.csv', index=False)
...
history_df.to_csv('test_qica_history.csv', index=False)
...
if len(al_df_out) > 0:
    al_df_out.to_csv('test_qica_al_candidates.csv', index=False)
print(f"\n[SAVED] test_qica_summary.csv  test_qica_history.csv  test_qica_al_candidates.csv")
...
plt.savefig('test_qica_comparison.png', dpi=150, bbox_inches='tight')
print("[SAVED] test_qica_comparison.png")
...
for r in all_results:
    ...
    inj_improvements = 0
    for ig in inj_gens:
        window = h['best_ppf'][ig:min(ig+10, len(h['best_ppf']))]
        if len(window) > 1 and min(window[1:]) < h['best_ppf'][ig]:
            inj_improvements += 1
    ...
    print(f"    Injections fired  : {r['total_injections']}  (improved next 10 gens: {inj_improvements}/{max(len(inj_gens),1) if inj_gens else 'N/A'})")
...
print("\n" + "=" * 80)
print("SEND BACK: test_qica_summary.csv  +  test_qica_comparison.png")
